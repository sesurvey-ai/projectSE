...
class _SurveyFormScreenState extends State<SurveyFormScreen> with WidgetsBindingObserver {
  ...
  List<String> _sectionMissing(_SView v) {
    ...
    switch (v) {
      ...
      case _SView.s5:
        ...
        //
        // user 31/08/69: บังคับเพิ่มเมื่อ **ฝ่ายประมาท = "รถประกันฝ่ายถูก" หรือ "ฝ่ายถูกและผิด"**
        // ถึงจะยังไม่ได้กรอกคู่กรณีในหมวด 6 ก็ตาม — สองกรณีนี้แปลว่ามีคู่กรณีที่ต้องไปเรียกเก็บ
        // อยู่แล้วโดยนิยาม ปล่อยผ่านไปก็ไปค้างให้หัวหน้าเติมเองตอนตรวจทุกใบ
        // (เงื่อนไข "มีคู่กรณี" ของเดิมยังอยู่ครบ ไม่ได้ผ่อนลง)
        if (_opponents.isNotEmpty || _faultIs('คู่กรณีผิด') || _faultIs('ฝ่ายถูกและผิด')) {
          base.addAll(miss([
  ...
  Widget _faultDropdown() {
    ...
    const opts = <String, String>{
      ...
      // user 31/08/69: เรียกจากมุมรถประกันให้เหมือนตัวเลือกอื่น ("คู่กรณีผิด" = "รถประกันฝ่ายถูก"
      // เรื่องเดียวกัน คนละมุมมอง) — EMCS ก็ถือเป็นช่องเดียวกัน (rdoAcc_Cause01)
      'คู่กรณีผิด': 'รถประกันฝ่ายถูก',
  ...
  Widget _opoClaimChecks() {
    ...
    // จุดแดงขึ้นตามเงื่อนไขจริง ไม่ใช่ติดตายไว้ — ต้องตรงกับที่ตัวตรวจก่อนส่งงานใช้
    // (ถ้าบังคับแต่ไม่ขึ้นจุด ช่างจะรู้ตัวตอนกดส่งแล้วไม่ผ่าน แล้วต้องไล่หาเองว่าช่องไหน)
    final claimRequired = _opponents.isNotEmpty || _faultIs('คู่กรณีผิด') || _faultIs('ฝ่ายถูกและผิด');
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _fieldLabel('การเรียกร้องค่าเสียหายจากคู่กรณี', req: claimRequired),
      for (final e in opts.entries)
        GestureDetector(
