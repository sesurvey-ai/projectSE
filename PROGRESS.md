...
### ปรับปรุงระบบจัดเก็บไฟล์ + ยืนยันถึงที่เกิดเหตุ + UI (31 มี.ค. — 2 เม.ย. 2026 — v1.5.22 ถึง v1.5.23)

**Mobile — ถ่ายรูปยืนยันถึงที่เกิดเหตุ:**
- [x] ก่อนกด "เริ่มสำรวจ" ต้องถ่ายรูปยืนยันว่าถึงที่เกิดเหตุแล้ว
- [x] กล่องสีส้ม "ถ่ายรูปยืนยันถึงที่เกิดเหตุ" → ถ่ายรูป → อัปโหลด → กล่องเขียว + ปุ่ม "เริ่มสำรวจ" ปรากฏ
- [x] บันทึกเวลาถึงที่เกิดเหตุ (`acc_survey_arrive_date`) ใน DB
- [x] แสดง preview รูปหลังยืนยัน (local file หรือ จาก server)
- [x] ถ้าเคยถ่ายแล้ว → เช็คจาก API อัตโนมัติ ไม่ต้องถ่ายซ้ำ

**Mobile — โฟลเดอร์จัดเก็บรูปบนเครื่อง:**
- [x] เก็บรูปที่ `Download/SE_Survey/{เลขเคลม}/{เลขเรื่องเซอร์เวย์}/`
- [x] ดาวน์โหลดรูป OCR (หน้าการ์ด) จาก server ลงโฟลเดอร์อัตโนมัติ
- [x] รูปยืนยัน (arrival.jpg) + รูปสำรวจ (survey_*.jpg) เก็บในโฟลเดอร์เดียวกัน
- [x] เปิดดูผ่าน File Manager ได้ที่ Download > SE_Survey
- [x] Permission: WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE, MANAGE_EXTERNAL_STORAGE

**Mobile — ปุ่มดูหน้าการ์ดในฟอร์มสำรวจ:**
- [x] FAB ไอคอนการ์ด (mini) เหนือปุ่ม save draft
- [x] กดเปิด DraggableScrollableSheet แสดงรูปหน้าการ์ด (OCR เท่านั้น)
- [x] กดรูป → เต็มจอ PageView + InteractiveViewer (swipe + pinch-to-zoom)
- [x] ปิด sheet → กรอกฟอร์มต่อได้ทันที

**Mobile — หน้ารายละเอียดงาน:**
- [x] เปลี่ยน "รายละเอียดรถยนต์" เป็น "รายละเอียด"
- [x] Section พับได้: บริษัทประกัน/เคลม, กรมธรรม์, รถยนต์, หน้าการ์ด

**Server — โฟลเดอร์จัดเก็บรูป:**
- [x] โครงสร้าง: `uploads/{เลขเคลม}/{เลขเรื่องเซอร์เวย์}/`
- [x] รูป OCR ย้ายเข้าโฟลเดอร์ตอนสร้างเคส (rename ไม่ copy)
- [x] OCR ไม่เก็บรูปบน server — อ่านแล้ว return ข้อมูล + เก็บชั่วคราว → ย้ายตอนสร้างเคส
- [x] มือถือส่งงาน → อัปโหลดทั้งโฟลเดอร์ผ่าน `POST /api/cases/:id/upload-folder`
- [x] ลบรูปเก่าในโฟลเดอร์ย่อยก่อนใส่รูปใหม่จากมือถือ
- [x] ลบรูปค้างที่ uploads/ root
- [x] เลขเคลมเดียวกันหลายครั้ง → แยกโฟลเดอร์ย่อยตามเลขเรื่องเซอร์เวย์
- [x] `survey_photos` บันทึกจากไฟล์ในโฟลเดอร์ที่อัปโหลดมา

**Backend — API ใหม่:**
- [x] `POST /api/cases/:id/arrival` — ยืนยันถึงที่เกิดเหตุ + บันทึกเวลา
- [x] `GET /api/cases/:id/arrival` — เช็ครูปยืนยัน
- [x] `POST /api/cases/:id/folder` — สร้างโฟลเดอร์บน server
- [x] `POST /api/cases/:id/upload-folder` — อัปโหลดทั้งโฟลเดอร์จากมือถือ

**Web — Call Center ปรับปรุง:**
- [x] ไทยไพบูลย์: เพิ่ม 3 ช่องกรอกข้อมูลหลัก (เลขเคลม, เลขเรื่องเซอร์เวย์, เลขรับแจ้ง)
- [x] ตารางข้อมูลแสดงเสมอ — ไม่ต้อง OCR ก่อนก็กรอกเองได้
- [x] Sidebar เปลี่ยนชื่อเป็น "รับแจ้งอุบัติเหตุ"
- [x] ย้าย "งานทั้งหมดในระบบ" ขึ้นแถวเดียวกับ "สวัสดี"

**Web — ตารางเคส (callcenter + inspector):**
- [x] คอลัมน์ตรงกัน: สถานะ, เลขเคลม, เลขเซอร์เวย์, เลขรับแจ้ง, ช่างสำรวจ, ครั้งที่
- [x] สีสถานะตรงกัน (เทา/ส้ม/น้ำเงิน/เขียว)
- [x] visit_count คำนวณจาก claim_no (ROW_NUMBER PARTITION BY)

**Web — ระบบตรวจสอบ:**
- [x] ส่วน "การตรวจสอบ" + "ค่าใช้จ่าย" กรอกได้เลยไม่ต้องกดแก้ไข
- [x] ปุ่ม "บันทึก" แสดงเสมอ + ปุ่ม "อนุมัติ" แยกต่างหาก
- [x] ลบ hardcode ข้อมูลซ้ำ (ผลการดำเนินงาน, ค่าบริการ)
