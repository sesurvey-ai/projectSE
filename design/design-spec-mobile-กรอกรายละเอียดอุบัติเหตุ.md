...
3. **Chip grid (wrap)** — ตัวเลือกที่ควรเห็นครบ: ประเภทเคลม (4), ประเภทรถ (7 พร้อมไอคอน), EV (6 รวม "ไม่ใช่ EV"), การติดตามงาน (3), ระดับบาดเจ็บ (6 สี)
4. **Radio/Selection card** — ตัวเลือกเชิงกฎหมายที่ต้องเห็นครบพร้อมคำอธิบาย: ฝ่ายประมาท (7 การ์ด, `#faultCards`)
...
3. **ประเภทรถ** [req ●] — chips (`#chipsCType`, CTYPES 7 พร้อม emoji)
...
8. **รถยนต์ไฟฟ้า (EV)** (ไม่บังคับ) — chips (`#chipsEV`); เลือก BEV/HEV/PHEV/FCEV/MEV → โผล่ `#evExtra`: หมายเลขแบตเตอรี่ [req ●], วันเริ่มใช้งานแบตเตอรี่, หมายเลขเครื่องชาร์จ
...
1. **วัน-เวลาเกิดเหตุ (พ.ศ.)** [req ●] — date (`accDate`); ใต้มี chips ทางลัด (ro-hide): ⏱ ตอนนี้ / วันนี้ / เมื่อวาน
2. **สถานที่เกิดเหตุ** [req ●] — ปุ่ม "📍 ใช้ตำแหน่งปัจจุบัน (GPS)" (ro-hide) + แผนที่จำลอง `#mapMock` (ซ่อนเริ่มต้น) + input text ถนน/จุดสังเกต (`#fAccPlace`) + picker location (`#vfAccLoc`); warn `#gpsWarn` "GPS อยู่ห่างจากจังหวัดที่เลือกมาก"
...
5. **รายละเอียดการเกิดเหตุ** [req ●] — textarea (`#fAccDetail`); label hint "เรียบเรียงทีหลังได้ — บังคับตอนส่งรายงาน"; ปุ่ม (ro-hide): 🎙 พูดเพื่อพิมพ์, 📋 เทมเพลตประโยคเปิด
6. **ฝ่ายประมาท** [req ●] — `#faultCards` (7 การ์ด) + `#faultOpoPick` (โผล่เมื่อ fault="รถคู่กรณีเป็นฝ่ายผิด" → บังคับเลือกคันที่ผิด `S.faultOpo`)
7. **ผู้แจ้งเหตุ** (ไม่บังคับ) — input text (`#fAccCall`)
8. **ผู้สำรวจภัย** [req ●] — input text (`#fAccSurv`, **prefill = "SV104 สมชาย สำรวจดี"**); fhelp "เติมจากโปรไฟล์ผู้ล็อกอินอัตโนมัติ — แก้ได้กรณีสำรวจแทน/งานส่งต่อ"
...
13. **การเรียกร้องค่าเสียหายจากคู่กรณี** (ไม่บังคับ) — `#claimChecks` (checkbox list, OPO_CLAIMS 5 ข้อ) + `#claimTotal` แสดงยอดรวม; ข้อ "รับเงินจำนวน" ติ๊กแล้วต้องมี `rec` (ยอดเรียกร้องรวม); ถ้า pay > rec → error `#payErr` "รับเงินจากคู่กรณีเกินยอดเรียกร้องรวม" **(เป็น error block ส่ง)**
