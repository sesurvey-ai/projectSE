...
check('ปัดเป็นจำนวนเต็มเมื่อเป็นจุดกลางอำเภอ',
      /~\{Math\.round\(Number\(s\.distance\)\)\} กม\./.test(web));
check('บอกว่าวัดจากจุดกลางอำเภอ ไม่ใช่จุดเกิดเหตุ', web.includes('ไม่ใช่จุดเกิดเหตุจริง'));
/**
 * ⛔ เคสที่มีพิกัดจริงมากับตัว (เส้นมือถือ ละเอียดถึงปากซอย) **ห้ามโดนคำเตือนนี้ด้วย** —
 *    ติดป้าย "โดยประมาณ" ทับพิกัดที่แม่นอยู่แล้ว คนจ่ายงานจะเลิกเชื่อตัวเลขที่เชื่อได้
 *    (เจอตอนตรวจกับเคสจริง #100/#101 บน prod 25/08/69 ซึ่งเป็นพิกัดระดับถนนจริง)
 */
check('คำเตือนขึ้นเฉพาะตอนใช้จุดกลางอำเภอ', /const approx = coordSource === 'district'/.test(web));
check('พิกัดที่มากับเคสยังโชว์ทศนิยมตามเดิม',
      /approx \? \(/.test(web) && /toFixed\(1\)\} กม\./.test(web));
