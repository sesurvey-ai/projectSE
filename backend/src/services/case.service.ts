...
export const caseService = {
  ...
  async getById(caseId: number) {
    ...
    /**
     * ไม่มีพิกัดแต่รู้อำเภอ → เติมจุดกลางอำเภอให้ (งานไทยไพบูลย์ที่การ์ดไม่พิมพ์พิกัด
     * เป็นแบบนี้ทุกใบ) หน้าจ่ายงานจะได้เรียงช่างตามระยะทางได้เหมือนกัน
     *
     * ⚠️ ต้องส่ง `incident_coord_source` ไปด้วยเสมอ เพราะ 2 ทางนี้**เชื่อถือได้ไม่เท่ากัน**:
     *    `'case'`     = พิกัดที่มากับเคส — บางใบเป็นที่เกิดเหตุจริงระดับปากซอย
     *    `'district'` = เราคำนวณเอง ละเอียดแค่ระดับอำเภอ (กว้างได้หลายสิบกิโลเมตร)
     *    ⛔ หน้าจอต้องเขียนคำเตือน "จุดกลางอำเภอ" **เฉพาะกรณีหลัง** — เหมารวมทั้งสองแบบ
     *      คือโกหกคนใช้ในทางกลับกัน (ทำให้เขาไม่เชื่อพิกัดจริงที่แม่นอยู่แล้ว)
     *    ไม่เขียนลง DB เพราะเป็นค่าที่คำนวณใหม่ได้เสมอ และจะแยกไม่ออกว่าอันไหนของจริง
     */
