...
// วันที่ = YYYY-MM-DD, เวลา timestamp = แปลงเป็นเวลาไทย (Asia/Bangkok)
const COLS = `
  ...
  to_char((lr.reviewed_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Bangkok', 'YYYY-MM-DD HH24:MI') AS reviewed_at,
  to_char((lr.created_at  AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Bangkok', 'YYYY-MM-DD HH24:MI') AS created_at`;
    ...
    const { rows } = await db.query(
      `INSERT INTO leave_requests (user_id, leave_type, start_date, end_date, reason)
       ...
       RETURNING
         ...
         to_char((reviewed_at AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Bangkok', 'YYYY-MM-DD HH24:MI') AS reviewed_at,
         to_char((created_at  AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Bangkok', 'YYYY-MM-DD HH24:MI') AS created_at`,
