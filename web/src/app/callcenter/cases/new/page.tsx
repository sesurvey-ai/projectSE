      ...
      <form onSubmit={handleSubmit}>
        ...
        <table className="w-full border-collapse border border-gray-200 bg-white text-[12px]">
          <tbody>
            ...
            <tr>
              <td className={L}>บริษัทประกัน</td>
              <td className={V} colSpan={2}>
                <select value={insuranceCompany} onChange={e => setInsuranceCompany(e.target.value)} className={`${I} bg-transparent`}>
                  <option value="">- เลือก -</option>
                  <option value="ไอโออิกรุงเทพประกันภัย">ไอโออิกรุงเทพประกันภัย</option>
                  <option value="บริษัท ไทยไพบูลย์ประกันภัย จำกัด (มหาชน)">ไทยไพบูลย์ประกันภัย</option>
                </select>
              </td>
              <td className={L}>สาขา</td>
              <td className={V} colSpan={2}><input value={f('insurance_branch')} onChange={e => s('insurance_branch', e.target.value)} className={I} placeholder="กรุงเทพ" /></td>
            </tr>
